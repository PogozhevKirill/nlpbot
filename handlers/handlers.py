...
from db.mongo import mongo_users, do_find_one, do_insert_one, do_find_many, do_delete_one


async def enter_username(data: dict):
    current_char = await do_find_one({'_id': data['message']['chat']['id']}, mongo_users)
    if current_char is None:
        await do_insert_one({'_id': data['message']['chat']['id'],
                             'name': data['message']['text']}, mongo_users)
        return 'Персонаж **{}** успешно создан!'.format(data['message']['text'])
    return None


async def get_users_list():
    users_list = await do_find_many({}, mongo_users)
...
async def delete_user(data: dict):
    ...
    if current_char is not None:
        await do_delete_one({'_id': data['message']['chat']['id']}, mongo_users)
